...
Use `scripts/release_workflow_support.py` relative to the installed skill-master root. If it is missing, stop with an actionable `Incomplete` result; do not improvise replacement commands.
...
See `references/release-workflow-evidence.md` for the shared record and failure semantics.
